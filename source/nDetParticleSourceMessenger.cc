...
/*void nDetImplantParticleSourceMessenger::addAllCommands(){
	addDirectory("/nDet/source/", "Particle Source Control");
...
void nDetImplantParticleSourceMessenger::SetNewChildValue(G4UIcommand* command, G4String newValue){ 
	size_t index;
	if(!findCommand(command, newValue, index)) return;
	if(index == 0){
		fActionI->Print(G4UIcommand::ConvertToInt(newValue));
	}
	else if(index == 1){
		fActionI->SetSourceDirection(G4UIcommand::ConvertTo3Vector(newValue));
	}
	else if(index == 2){
		fActionI->SetSourceType(newValue);
	}
	else if(index == 3){
		fActionI->SetBeamspotRadius(G4UIcommand::ConvertToDouble(newValue));
	}
	else if(index == 4){
		fActionI->SetBeamspotRadius0(G4UIcommand::ConvertToDouble(newValue));
	}
	else if(index == 5){
		fActionI->SetBeamspotType(newValue);
	}
	else if(index == 6){
		fActionI->SetIsotropicMode(G4UIcommand::ConvertToInt(newValue));
	}
	else if(index == 7){
		G4ThreeVector vec = G4UIcommand::ConvertTo3Vector(newValue);
		fActionI->SetEnergyLimits(vec.getX(), vec.getY());
	}
	else if(index == 8){
		fActionI->ReadReactionFile(newValue);
	}
	else if(index == 9){
		fActionI->ReadEnergyFile(newValue);
	}
	else if(index == 10){
		fActionI->AddDiscreteEnergy(newValue);
	}
	else if(index == 11){
		fActionI->Test(newValue);
	}
	else if(index == 12){
		fActionI->Reset();
	}
	else if(index == 13){
		fActionI->SetInterpolationMethod(newValue);
	}
	else if(index == 14){
		fActionI->SetBeamEnergy(G4UIcommand::ConvertToDouble(newValue));
	}
	else if(index == 15){
		fActionI->SetBeamEnergySigma(newValue);
	}
}
*/